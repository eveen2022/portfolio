...
import { buildPageMetadata, creativeWorkJsonLd, JsonLd } from "@/lib/seo";
...
export async function generateMetadata({
  ...
  return buildPageMetadata({
    ...
    image: project.image || siteConfig.seo.ogImage || siteConfig.photo || undefined,
    noIndex: siteConfig.seo.noIndex || project.noIndex,
              ...
              {project.techStack.map((tech) => {
                ...
                return (
                  <Badge key={tech.name} className="gap-1.5">
                    {techIcon && (
                      <techIcon.Icon className="size-3.5 shrink-0" />
                    )}
                    {tech.name}
                  </Badge>
                );
